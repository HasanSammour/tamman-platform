{{-- resources/views/specialist/treatment-plans/edit.blade.php --}}
@extends('layouts.app')

@section('title', __('Edit Treatment Plan') . ' - ' . __('Tamman'))

@section('page-title', __('Edit Treatment Plan'))

@section('content')
<div class="edit-plan-container">
    <div class="edit-plan-card animate-slide-up">
        ...
        <div class="form-header">
            ...
            <h2>{{ __('Edit Treatment Plan') }}</h2>
            <p>{{ __('Update treatment plan details and manage tasks') }}</p>
        </div>
        ...
        <form id="treatmentPlanForm" class="treatment-plan-form">
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="patient_id">{{ __('Patient') }} <span class="required">*</span></label>
                        <div class="select-wrapper disabled">
                            <i class="fas fa-user select-icon"></i>
                            <select name="patient_id" id="patient_id" class="form-control" disabled>
                                <option value="{{ $plan->patient_id }}" selected>{{ $plan->patient->name }} ({{ $plan->patient->email }})</option>
                            </select>
                            <i class="fas fa-lock disabled-icon"></i>
                        </div>
                        <div class="form-hint">{{ __('Patient cannot be changed after plan creation') }}</div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            ...
                            <textarea name="description" id="description" class="form-control" rows="4"
                                placeholder="{{ __('Describe the overall goals and objectives of this treatment plan...') }}">{{ old('description', $plan->description) }}</textarea>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            ...
                            <input type="date" name="end_date" id="end_date" class="form-control" 
                                value="{{ old('end_date', $plan->end_date ? \Carbon\Carbon::parse($plan->end_date)->format('Y-m-d') : '') }}">
                        </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="status">{{ __('Status') }} <span class="required">*</span></label>
                        <div class="select-wrapper">
                            <i class="fas fa-flag-checkered select-icon"></i>
                            <select name="status" id="status" class="form-control" required>
                                <option value="active" {{ $plan->status == 'active' ? 'selected' : '' }}>{{ __('Active') }}</option>
                                <option value="completed" {{ $plan->status == 'completed' ? 'selected' : '' }}>{{ __('Completed') }}</option>
                                <option value="cancelled" {{ $plan->status == 'cancelled' ? 'selected' : '' }}>{{ __('Cancelled') }}</option>
                            </select>
            ...
            <div class="form-section tasks-section">
                <div class="section-title">
                    ...
                    <p>{{ __('Manage tasks for this treatment plan. Completed tasks cannot be modified.') }}</p>
                </div>
                    ...
                    <div class="tasks-list" id="tasks-list">
                        ...
                        <div class="task-card {{ $task->is_completed ? 'completed' : '' }}" data-task-id="{{ $task->id }}">
                            <div class="task-header">
                                ...
                                <button type="button" class="btn-remove-task" onclick="removeTask(this, {{ $task->id }})">
                                    <i class="fas fa-trash-alt"></i>
                                </button>
                                @else
                                <span class="completed-lock"><i class="fas fa-lock"></i> {{ __('Completed') }}</span>
                                @endif
                            </div>
                            <input type="hidden" name="existing_tasks[]" value="{{ $task->id }}" class="existing-task-id">
                            
                            <div class="task-form-row full-width">
                                <div class="task-form-group">
                                    <label>{{ __('Task Title') }} <span class="required">*</span></label>
                                    <div class="input-wrapper">
                                        <i class="fas fa-tasks input-icon"></i>
                                        <input type="text" name="tasks[{{ $task->id }}][title]" class="form-control task-title" 
                                            value="{{ $task->title }}" {{ $task->is_completed ? 'disabled' : '' }} required>
                                    </div>
                                </div>
                            </div>
                            <div class="task-form-row full-width">
                                <div class="task-form-group">
                                    ...
                                    <div class="input-wrapper">
                                        <i class="fas fa-align-left input-icon textarea-icon"></i>
                                        <textarea name="tasks[{{ $task->id }}][description]" class="form-control" rows="2" 
                                            {{ $task->is_completed ? 'disabled' : '' }}>{{ $task->description }}</textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="task-form-row">
                                <div class="task-form-group">
                                    ...
                                    <div class="input-wrapper">
                                        <i class="fas fa-calendar-alt input-icon"></i>
                                        <input type="date" name="tasks[{{ $task->id }}][due_date]" class="form-control" 
                                            value="{{ $task->due_date ? \Carbon\Carbon::parse($task->due_date)->format('Y-m-d') : '' }}"
                                            {{ $task->is_completed ? 'disabled' : '' }}>
                                    </div>
                                </div>
                                <div class="task-form-group">
                                    <label>{{ __('Points Reward') }} <span class="required">*</span></label>
                                    <div class="points-input-wrapper">
                                        <input type="number" name="tasks[{{ $task->id }}][points_reward]" class="form-control task-points" 
                                            min="3" max="15" step="1" value="{{ $task->points_reward }}" 
                                            {{ $task->is_completed ? 'disabled' : '' }} required>
                            ...
                            <div class="task-completed-info">
                                <i class="fas fa-check-circle"></i> {{ __('Completed on') }}: {{ \Carbon\Carbon::parse($task->completed_at)->translatedFormat('M d, Y h:i A') }}
                            </div>
                            @endif
                        </div>
                        @endforeach
            ...
            <div class="form-actions">
                <a href="{{ route('specialist.treatment-plans.show', $plan->id) }}" class="btn-cancel">
...
<script>
    let taskCounter = {{ $plan->tasks->count() }};
    ...
    function createTaskTemplate(taskNumber, taskData = null) {
        const taskId = 'new_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
        ...
        return `
            <div class="task-card" data-task-id="${taskId}" data-is-new="true">
                <div class="task-header">
                    <span class="task-number">${taskNumber}</span>
                    <button type="button" class="btn-remove-task" onclick="removeNewTask(this)">
                        <i class="fas fa-trash-alt"></i>
                    </button>
                </div>
                <div class="task-form-row full-width">
                    <div class="task-form-group">
                        ...
                        <div class="input-wrapper">
                            <i class="fas fa-tasks input-icon"></i>
                            <input type="text" name="new_tasks[${taskId}][title]" class="form-control task-title" 
                ...
                <div class="task-form-row full-width">
                    <div class="task-form-group">
                        ...
                        <div class="input-wrapper">
                            <i class="fas fa-align-left input-icon textarea-icon"></i>
                            <textarea name="new_tasks[${taskId}][description]" class="form-control" rows="2" 
                ...
                <div class="task-form-row">
                    <div class="task-form-group">
                        ...
                        <div class="input-wrapper">
                            <i class="fas fa-calendar-alt input-icon"></i>
                            <input type="date" name="new_tasks[${taskId}][due_date]" class="form-control" value="${dueDate}">
                        </div>
                    </div>
                    <div class="task-form-group">
                        <label>${escapeHtml('{{ __("Points Reward") }}')} <span class="required">*</span></label>
                        <div class="points-input-wrapper">
                            <input type="number" name="new_tasks[${taskId}][points_reward]" class="form-control task-points" 
    ...
    // Remove existing task (mark for deletion)
    ...
    // Remove new task (not yet saved)
    window.removeNewTask = function(button) {
        const taskCard = button.closest('.task-card');
        if (taskCard) {
            taskCard.remove();
            updateTaskNumbers();
    ...
    // Initialize points validation for existing tasks
    document.querySelectorAll('.task-points').forEach(input => {
    ...
    function validateRequiredFields() {
        ...
        if (statusInput && !statusInput.value) {
            showError('status', '{{ __("Please select a status") }}');
            isValid = false;
        } else {
            clearError('status');
        }
        
        // Task titles for existing tasks (not completed)
        document.querySelectorAll('.task-card:not(.completed) .task-title').forEach(titleInput => {
        ...
        // Task titles for new tasks
        document.querySelectorAll('.task-card[data-is-new="true"] .task-title').forEach(titleInput => {
            if (!titleInput.value.trim()) {
                titleInput.classList.add('error');
                isValid = false;
            }
    ...
    function validatePointsFields() {
        let isValid = true;
        document.querySelectorAll('.task-card:not(.completed) .task-points, .task-card[data-is-new="true"] .task-points').forEach(input => {
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            deletedTasks.forEach(taskId => {
                formData.append('deleted_tasks[]', taskId);
            });
            
            // Process existing tasks (not completed)
            let taskIndex = 0;
            document.querySelectorAll('.task-card:not(.completed):not([data-is-new="true"])').forEach(taskCard => {
                const taskId = taskCard.querySelector('.existing-task-id')?.value;
                ...
                if (taskId && titleInput && titleInput.value.trim()) {
                    formData.append(`tasks[${taskIndex}][id]`, taskId);
                    formData.append(`tasks[${taskIndex}][title]`, titleInput.value.trim());
                    formData.append(`tasks[${taskIndex}][description]`, descTextarea ? descTextarea.value : '');
                    formData.append(`tasks[${taskIndex}][due_date]`, dateInput ? dateInput.value : '');
                    formData.append(`tasks[${taskIndex}][points_reward]`, pointsInput ? parseInt(pointsInput.value) : 5);
                    taskIndex++;
                }
            });
            
            // Process new tasks
            document.querySelectorAll('.task-card[data-is-new="true"]').forEach(taskCard => {
                const titleInput = taskCard.querySelector('.task-title');
                const descTextarea = taskCard.querySelector('textarea[name*="[description]"]');
                const dateInput = taskCard.querySelector('input[type="date"]');
                const pointsInput = taskCard.querySelector('.task-points');
            ...
            try {
                const response = await fetch('{{ route("specialist.treatment-plans.update", $plan->id) }}', {
                    method: 'POST',
                    headers: {
