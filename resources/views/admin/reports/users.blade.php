...
@section('title', __('Users Report') . ' - ' . __('Tamman'))

@section('page-title', __('Users Report'))

@section('content')
    <div class="users-report-wrapper">
        <!-- Stats Summary -->
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-data">
                    <h3 id="summaryTotal">{{ number_format($globalStats['total_users'] ?? 0) }}</h3>
                    <p>{{ __('Total Users') }}</p>
                </div>
            </div>
            <div class="stat-card">
                ...
                <div class="stat-data">
                    <h3 id="summaryActive">{{ number_format($globalStats['active_users'] ?? 0) }}</h3>
                    <p>{{ __('Active') }}</p>
                </div>
            </div>
            <div class="stat-card">
                ...
                <div class="stat-data">
                    <h3 id="summaryDonors">{{ number_format($globalStats['donors'] ?? 0) }}</h3>
                    <p>{{ __('Donors') }}</p>
                </div>
            </div>
            <div class="stat-card">
                ...
                <div class="stat-data">
                    <h3 id="summarySessions">{{ number_format($globalStats['total_sessions'] ?? 0) }}</h3>
        ...
        <div class="filters-container">
            <!-- Row 1: Date Range & Status & Gender & Donor -->
            <div class="filters-row">
                ...
                <select id="filterDonor" class="filter-select">
            ...
            <div class="filters-row">
                <div class="search-field">
                    ...
                    <input type="text" id="filterSearch" placeholder="{{ __('Search by name, email or phone...') }}">
                    ...
                    <table class="users-table">
                        <thead>
                            <tr>
                                ...
                                <th data-sort="gender">{{ __('Gender') }}</th>
                                <th data-sort="created_at" class="sortable">{{ __('Registered') }} <i
    ...
    @push('styles')
        <style>
            ...
            .loading-row td .loading-container p {
                margin: 0;
                color: #6b7280;
                font-size: 0.85rem;
            }
    ...
    @push('scripts')
        ...
        <script>
            ...
            const summaryDonors = document.getElementById('summaryDonors');
            ...
            function updateFilters() {
                ...
                filters.donor = filterDonor.value;
            ...
            async function loadReport() {
                ...
                try {
                    ...
                    const response = await fetch(`{{ route("admin.reports.users.data") }}?${params}`, {
            ...
            function updateStats(stats) {
                if (stats) {
                    if (summaryTotal) summaryTotal.textContent = (stats.total_users || 0).toLocaleString();
                    if (summaryActive) summaryActive.textContent = (stats.active_users || 0).toLocaleString();
                    if (summaryDonors) summaryDonors.textContent = (stats.donors || 0).toLocaleString();
                    if (summarySessions) summarySessions.textContent = (stats.total_sessions || 0).toLocaleString();
                }
            }

            function renderTable(users) {
                if (!users || users.length === 0) {
                    tableBody.innerHTML = `<tr class="empty-row"><td colspan="11"><div style="text-align:center;padding:40px;"><i class="fas fa-users" style="font-size:3rem;color:#c4b5fd;"></i><p style="margin-top:10px;">{{ __('No users found') }}</p></div></td></tr>`;
                    return;
                }
                ...
                tableBody.innerHTML = users.map(user => {
                    ...
                    if (user.profile_image_url) {
                        avatarHtml = `<img src="${user.profile_image_url}" alt="${escapeHtml(user.name)}" onerror="this.style.display='none';this.parentElement.textContent='${escapeHtml((user.name?.charAt(0) || 'U').toUpperCase())}'">`;
                    } else {
                    ...
                    let genderText = '—';
                    if (user.gender === 'male') genderText = '{{ __("Male") }}';
                    else if (user.gender === 'female') genderText = '{{ __("Female") }}';
                    else if (user.gender === 'other') genderText = '{{ __("Other") }}';
                    ...
                    return `
                            <tr>
                                ...
                                <td>${user.is_donor ? '<span class="badge badge-donor"><i class="fas fa-hand-holding-heart"></i> {{ __("Donor") }}</span>' : '<span class="badge badge-inactive">{{ __("No") }}</span>'}</td>
                                <td>${user.is_active ? '<span class="badge badge-active"><i class="fas fa-check-circle"></i> {{ __("Active") }}</span>' : '<span class="badge badge-inactive"><i class="fas fa-ban"></i> {{ __("Inactive") }}</span>'}</td>
                            </tr>
                        `;
                }).join('');
            }
            ...
            function renderPagination(data) {
                ...
                if (total === 0) {
                    tableInfo.innerHTML = '{{ __("No users found") }}';
                ...
                tableInfo.innerHTML = `{{ __('Showing') }} ${from} - ${to} {{ __('of') }} ${total.toLocaleString()} {{ __('users') }}`;
            ...
            async function exportReport() {
                ...
                try {
                    window.open(`{{ route("admin.reports.users.export") }}?${params}`, '_blank');
                    Swal.fire({
                        icon: 'success',
